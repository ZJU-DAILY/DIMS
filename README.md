...
| DIMS-NL       | - DIMS with no local homogeneous partition strategy          | -        |
| DIMS-P        | - DIMS with the most efficient in-memory pivot-based metric index | -        |
|               |                                                              |          |
...
| ----------- | ---------- | -------- | ---------------- | --------------------------------------------- |
| Words       | 611,756    | 1~34     | Edit Dis         | http://icon.shef.ac.uk/Moby/                  |
| T-Loc       | 10,000,000 | 2        | $L_{2}$-norm     | http://www.vldb.org/pvldb/vol12/p99-ghosh.pdf |
| Vec         | 100,000    | 300      | Word Cosine Dis. | http://crscardellino.me/SBWCE/                |
| Color       | 1,000,000  | 282      | $L_{1}$-norm     | http://cophir.isti.cnr.it/                    |

The dataset file is in txt format and can optionally be read from HDFS in a cluster environment or from local directory examples/src/main/resources/ in standalone mode. Each row in the dataset represents one piece of data, and every dimension within each row is split by spaces. For example, see examples/src/main/resources/mpeg_example.txt.
