...
import org.apache.spark.sql.catalyst.expressions.dims.{DIMSSimilarityWithKNNExpression, DIMSSimilarityRangeExpression, DIMSDeleteExpression, DIMSInsertExpression}
...
import org.apache.spark.sql.catalyst.expressions.dims.common.shape.{Point => DIMSPoint}
...
import org.apache.spark.sql.execution.mchord.exec.search.{MetricSimilarityWithKNNSearchExec, MetricRangeSearchExec, MetricDeleteExec, MetricInsertExec}
import org.apache.spark.sql.execution.amds.exec.{AMDSSimilarityWithKNNSearchExec, AMDSRangeSearchExec, AMDSDeleteExec, AMDSInsertExec}
import org.apache.spark.sql.execution.dims.exec.search.{DIMSSimilarityWithKNNSearchExec, DIMSRangeSearchExec, DIMSDeleteExec, DIMSInsertExec}
...
abstract class SparkStrategies extends QueryPlanner[SparkPlan] {
  ...
  object DIMSScans extends Strategy {

    def apply(plan: LogicalPlan): Seq[SparkPlan] = plan match {


      case PhysicalOperation(projectList, filters, relation)
        if filters.exists(e => e.isInstanceOf[DIMSSimilarityWithKNNExpression]) =>
        val expressionWithThreshold = filters.find(e =>
          e.isInstanceOf[DIMSSimilarityWithKNNExpression]).get
          .asInstanceOf[DIMSSimilarityWithKNNExpression]
        val count = expressionWithThreshold.count
        val expression = expressionWithThreshold.similarity
        val function = expression.function
        val (leftQuery, rightKey) = expression.met1 match {
          case Literal(v, _) => (v.asInstanceOf[DIMSPoint[Any]], expression.met2)
          case _ => (expression.met2.asInstanceOf[Literal].value.asInstanceOf[DIMSPoint[Any]],
            expression.met1)
        }
        val exec = DIMSSimilarityWithKNNSearchExec(leftQuery,
          rightKey, function, count, relation, planLater(relation))

        pruneFilterProject(
          projectList,
          filters,
          fs => fs.filter(f => !f.isInstanceOf[DIMSSimilarityWithKNNExpression]),
          _ => exec) :: Nil

      case PhysicalOperation(projectList, filters, relation)
        if filters.exists(e => e.isInstanceOf[DIMSSimilarityRangeExpression]) =>
        val circleExpression = filters.find(e =>
          e.isInstanceOf[DIMSSimilarityRangeExpression]).get
          .asInstanceOf[DIMSSimilarityRangeExpression]
        val expression = circleExpression.similarity
        val center = circleExpression.center
        val radius = circleExpression.radius

        val exec = DIMSRangeSearchExec(center,
          expression, radius, relation, planLater(relation))

        pruneFilterProject(
          projectList,
          filters,
          fs => fs.filter(f => !f.isInstanceOf[DIMSSimilarityRangeExpression]),
          _ => exec) :: Nil

      case PhysicalOperation(projectList, filters, relation)
        if filters.exists(e => e.isInstanceOf[DIMSDeleteExpression]) =>
        val circleExpression = filters.find(e =>
          e.isInstanceOf[DIMSDeleteExpression]).get
          .asInstanceOf[DIMSDeleteExpression]
        val expression = circleExpression.similarity
        val center = circleExpression.center

        val exec = DIMSDeleteExec(center,
          expression, relation, planLater(relation))

        pruneFilterProject(
          projectList,
          filters,
          fs => fs.filter(f => !f.isInstanceOf[DIMSDeleteExpression]),
          _ => exec) :: Nil

      case PhysicalOperation(projectList, filters, relation)
        if filters.exists(e => e.isInstanceOf[DIMSInsertExpression]) =>
        val circleExpression = filters.find(e =>
          e.isInstanceOf[DIMSInsertExpression]).get
          .asInstanceOf[DIMSInsertExpression]
        val expression = circleExpression.similarity
        val center = circleExpression.center

        val exec = DIMSInsertExec(center,
          expression, relation, planLater(relation))

        pruneFilterProject(
          projectList,
          filters,
          fs => fs.filter(f => !f.isInstanceOf[DIMSInsertExpression]),
          _ => exec) :: Nil
