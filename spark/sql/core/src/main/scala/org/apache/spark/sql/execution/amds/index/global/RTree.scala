...
case class RTreeInternalEntry(mbr: Rectangle, node: RTreeNode) extends RTreeEntry {
  override def minDist(x: Any): Double = mbr.minDist(x)
...
object RTree {
  def apply(entries: Array[(Point[Any], Int)], max_entries_per_node: Int): RTree = {
    ...
    grouped.foreach(list => {
      ...
      val mbr = Rectangle(Point[Any](0, min), Point[Any](0, max))
