...
import org.apache.spark.sql.execution.dims.algorithms.MetricRangeAlgorithms
...
case class DIMSRangeSearchExec(leftQuery: Point[Any], rightKey: Expression,
                              threshold: Double,
  ...
  protected override def doExecute(): RDD[InternalRow] = {
    ...
    val answerRDD = search.search(sparkContext, leftQuery, rightDIMSRDD, threshold)
    end = System.currentTimeMillis()
    logWarning(s"DIMS Range Search Time: ${end - start} ms")
