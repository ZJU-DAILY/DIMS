...
import org.apache.spark.sql.catalyst.expressions.dims._
...
import org.apache.spark.sql.execution.dims.sql.{CreateDIMSIndexCommand, DropDIMSIndexCommand}
...
class Dataset[T] private[sql](
  ...
  */
  ...
  def createDIMSIndex(key: Column, indexName: String): Unit = {
    CreateDIMSIndexCommand.createDIMSIndex(sparkSession, logicalPlan, key.toString,
      indexName, None)
  }
   ...
   * Drop DIMS index
   */
  def dropDIMSIndex(key: Column): Unit = {
    DropDIMSIndexCommand.dropDIMSIndex(sparkSession, logicalPlan, key.toString)
  }
  ...
  /*
   * DIMS similarity with knn search
   */
  def dimsSimilarityWithKNNSearch(query: DIMSPoint[Any], key: Column,
                                 function: DIMSSimilarityFunction,
                                 count: Int): DataFrame = withPlan {
    val dimsSimilarityExpression = DIMSSimilarityExpression(function, Literal(query,
      DataTypes.NullType), key.expr)
    val condition = DIMSSimilarityWithKNNExpression(dimsSimilarityExpression, count)
    Filter(condition, logicalPlan)
  }
  ...
  /*
   * DIMS similarity with range query
   */

  def dimsSimilarityWithRangeSearch(center: DIMSPoint[Any], radius: Double,
                                   key: Column): DataFrame = withPlan {
    val condition = DIMSSimilarityRangeExpression(key.expr, center, radius)
    Filter(condition, logicalPlan)
  }
  ...
  */
  ...
  def dimsDelete(center: DIMSPoint[Any], key: Column): DataFrame = withPlan {
    val condition = DIMSDeleteExpression(key.expr, center)
    Filter(condition, logicalPlan)
  }
  ...
  def dimsInsert(center: DIMSPoint[Any], key: Column): DataFrame = withPlan {
    val condition = DIMSInsertExpression(key.expr, center)
    Filter(condition, logicalPlan)
  }
