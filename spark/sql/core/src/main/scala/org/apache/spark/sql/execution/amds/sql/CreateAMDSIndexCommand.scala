...
case class CreateAMDSIndexCommand(tableName: TableIdentifier, column: String, indexName: String)
  extends RunnableCommand {

  override def run(sparkSession: SparkSession): Seq[Row] = {
    val table = sparkSession.table(tableName)
    CreateAMDSIndexCommand.createAMDSIndex(sparkSession, table.logicalPlan, column, indexName,
      Some(tableName))
    Seq.empty[Row]
  }
}

object CreateAMDSIndexCommand extends Logging {
  def createAMDSIndex(sparkSession: SparkSession, logicalPlan: LogicalPlan, column: String,
    ...
    if (catalog.lookupAMDSIndex(tableName, indexName, project).isEmpty) {
      val child = sparkSession.sessionState.executePlan(logicalPlan).sparkPlan
      val indexedRelation = AMDSIndexedRelation(child, attribute)()
