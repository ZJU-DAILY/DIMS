...
import org.apache.spark.sql.execution.amds.algorithms.MetricRangeAlgorithms
import org.apache.spark.sql.execution.amds.sql.AMDSIternalRow
import org.apache.spark.sql.execution.{SparkPlan, UnaryExecNode}

case class AMDSRangeSearchExec(leftQuery: Point[Any], rightKey: Expression,
                               threshold: Double,
  ...
  protected override def doExecute(): RDD[InternalRow] = {
    ...
    val answerRDD = search.search(sparkContext, leftQuery, rightAMDSRDD, threshold)
    end = System.currentTimeMillis()
    logWarning(s"AMDS Range Search Time: ${end - start} ms")
