...
case class DropAMDSIndexCommand(tableName: TableIdentifier, indexName: String)
  extends RunnableCommand {

  override def run(sparkSession: SparkSession): Seq[Row] = {
    DropAMDSIndexCommand.dropAMDSIndex(sparkSession, indexName, Some(tableName))
    Seq.empty[Row]
  }
}

object DropAMDSIndexCommand {

  def dropAMDSIndex(sparkSession: SparkSession, indexName: String,
                    tableName: Option[TableIdentifier]): Unit = {
    val catalog = sparkSession.sessionState.catalog
    catalog.dropIndex(tableName, indexName)
  }

  def dropAMDSIndex(sparkSession: SparkSession, logicalPlan: LogicalPlan,
                    column: String): Unit = {
    val catalog = sparkSession.sessionState.catalog
    val attribute = {
