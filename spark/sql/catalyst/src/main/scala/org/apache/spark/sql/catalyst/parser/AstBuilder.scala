...
class AstBuilder(conf: SQLConf) extends SqlBaseBaseVisitor[AnyRef] with Logging {
  ...
  Literal = withOrigin(ctx) {
    Literal(MetricPoint[Any](ctx.coords.asScala.map(_.getText.toDouble).toArray), DataTypes.NullType)
  }

  /*
   * Create a metric data
   */
  override def visitMetricExpression(ctx: MetricExpressionContext):
  Literal = withOrigin(ctx) {
    Literal(
      MetricData(ctx.points.asScala.map(x => visitMetricPointExpression(x).value.asInstanceOf[MetricPoint[Any]]).toArray), DataTypes.NullType)
  }
  ...
  (ctx: MetricSimilarityExpressionContext): MetricSimilarityExpression = withOrigin(ctx) {
    val left = if (ctx.leftTable != null) {
      ...
      visitMetricExpression(ctx.leftTrajectory)
    }
    val right = if (ctx.rightTable != null) {
      expression(ctx.rightTable)
    } else {
      visitMetricExpression(ctx.rightTrajectory)
    }
    ctx.function match {
  ...
  /**
   * Create a metric similarity with knn expression
   */
