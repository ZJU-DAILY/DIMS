...
import org.apache.spark.sql.catalyst.expressions.dims.index.{IndexEntry => DIMSIndexEntry}
import org.apache.spark.sql.catalyst.expressions.dims.index.{IndexRegistry => DIMSIndexRegistry}
import org.apache.spark.sql.catalyst.expressions.dims.index.{IndexedRelation => DIMSIndexedRelation}
...
class SessionCatalog(
                      ...
                      dimsIndexRegistry: DIMSIndexRegistry,
                      conf: SQLConf,
  ...
  /**
   * create index on the table DIMS
   */
  def createIndex(tableName: Option[TableIdentifier], indexName: String, plan: LogicalPlan,
                  indexedRelation: DIMSIndexedRelation): Unit = {
    if (dimsIndexRegistry.lookupIndex(indexName, plan).nonEmpty) {
      logWarning("Index exists!")
    } else {
      dimsIndexRegistry.registerIndex(indexName, tableName.map(_.table), plan, indexedRelation)
    }
  }
   ...
   * lookup index dims
   */
  def lookupDIMSIndex(tableName: Option[TableIdentifier], indexName: String,
                     plan: LogicalPlan): Option[DIMSIndexEntry] = {
    dimsIndexRegistry.lookupIndex(indexName, plan)
  }
   ...
   * lookup index dims
   */
  def lookupDIMSIndex(plan: LogicalPlan): Option[DIMSIndexEntry] = {
    dimsIndexRegistry.lookupIndex(plan)
  }
   ...
   * drop index dims
   */
  def dropDIMSIndex(plan: LogicalPlan): Unit = {
    if (!dimsIndexRegistry.dropIndex(plan)) {
      throw new NoSuchIndexException(plan.nodeName)
    }
  }
   ...
   * drop index dims
   */
  def dropDIMSIndex(tableName: Option[TableIdentifier], indexName: String): Unit = {
    if (!dimsIndexRegistry.dropIndex(indexName, tableName.map(_.table))) {
      throw new NoSuchIndexException(indexName)
    }
  }
   ...
   * show indexes dims
   */
  def showDIMSIndexes(): Seq[String] = {
    dimsIndexRegistry.showIndexes()
  }
  ...
  def reset(): Unit = synchronized {
    ...
    amdsIndexRegistry.clear()
    dimsIndexRegistry.clear()
    tableRelationCache.invalidateAll()
